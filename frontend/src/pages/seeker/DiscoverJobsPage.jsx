...
export const DiscoverJobsPage = () => {
    ...
    const formatDate = (dateString) => {
        ...
        return isNaN(d.getTime()) ? 'Recently' : d.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
    };
        ...
        <div className="w-full">
            <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-['Playfair_Display'] font-bold text-[#1A1A1A] flex items-center gap-2">
                        ...
                        Discover Global & Local Jobs
                    </h1>
                    <p className="text-gray-500 mt-1">Explore opportunities in Sri Lanka and worldwide via Jooble.</p>
                </div>
                <div className="bg-[#E2B325]/10 px-4 py-2 border border-[#E2B325]/30 rounded flex items-center gap-2">
                    <span className="text-xs uppercase tracking-wider font-bold text-[#8B1A1A]">Powered by</span>
            ...
            <form onSubmit={handleSearch} className="bg-white p-6 shadow-sm border border-gray-100 mb-8 rounded-md">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex flex-col">
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Keywords</label>
                    ...
                    {/* Where */}
                    <div className="flex flex-col">
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Location</label>
                        <div className="relative">
                    ...
                    {/* Category */}
                    <div className="flex flex-col">
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1">Category</label>
                        <div className="relative">
            ...
            <div className="mb-6 flex justify-between items-center">
                <h2 className="text-lg font-bold text-[#1A1A1A]">
                    {isLoading ? 'Searching...' : `${totalCount.toLocaleString()} external jobs found`}
                </h2>
            </div>
            ...
            {!isLoading && !error && jobs.length === 0 && (
                <div className="bg-white border border-gray-100 p-12 text-center rounded">
                    ...
                    <h3 className="text-lg font-bold text-[#1A1A1A] mb-2">No external jobs found</h3>
                    <p className="text-gray-500 max-w-md mx-auto">
                        We couldn't find any results from Jooble matching your current search criteria in Sri Lanka. Try adjusting your keywords.
                    </p>
                        ...
                        <div key={job.id} className="bg-white p-6 border border-gray-100 shadow-sm hover:border-[#8B1A1A] transition-colors rounded group flex flex-col md:flex-row gap-6">
                            <div className="flex-1">
                                ...
                                <p className="text-[#1A1A1A] font-semibold text-sm mb-3">
                                    {job.company || 'Unknown Company'} <span className="mx-2 text-gray-300">•</span> {job.location || 'Location Not Specified'}
                                </p>
                            ...
                            <div className="flex flex-col items-end justify-center shrink-0 border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6 w-full md:w-auto">
                                ...
                                <span className="text-[10px] text-gray-400 mt-2 uppercase tracking-wide text-center w-full">External Listing</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {!isLoading && !error && totalCount > 0 && (
                <div className="mt-8 flex items-center justify-between border-t border-gray-200 pt-6">
                    <span className="text-sm text-gray-500">
                        Showing page {page} of {Math.ceil(totalCount / 10).toLocaleString()}
                    </span>
